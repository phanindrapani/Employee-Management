...
export const getProjectMilestones = async (req, res) => {
    try {
        ...
        const milestones = await Milestone.find({ projectId }).populate('assignedTeam', 'name').lean();
        res.json(milestones);
    } catch (error) {
...
export const getAllTeamMilestones = async (req, res) => {
    try {
        ...
        const projects = await Project.find({ assignedTeams: teamId }).select('_id name').lean();
