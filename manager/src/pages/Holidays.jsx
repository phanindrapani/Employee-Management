...
const Holidays = () => {
    const { user } = useAuth();
    const [holidays, setHolidays] = useLocalStorage(`manager_holidays_list_${user?._id}`, []);
    const [myLeaves, setMyLeaves] = useLocalStorage(`manager_my_leaves_list_${user?._id}`, []);
    const [loading, setLoading] = useState(holidays.length === 0 && myLeaves.length === 0);

    const fetchData = useCallback(async () => {
        try {
            const [holidaysRes, myLeavesRes] = await Promise.allSettled([
                API.get('/holidays'),
