...
const Profile = () => {
    const { user: authUser, login } = useAuth(); // login used to update context
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setProfile(data);
            setIsEditing(false);
            // Optionally update global auth context if needed, though usually handled by re-login or dedicated update
            // For now, local state update is sufficient for the view
        } catch (err) {
                ...
                <div className="lg:col-span-1">
                    <div className="bg-white p-8 rounded-[24px] shadow-sm border border-slate-100 flex flex-col items-center text-center">
                        <div className="relative mb-6 group">
                            <div className="w-32 h-32 bg-[#0B3C5D] text-white rounded-full flex items-center justify-center text-4xl font-black shadow-xl ring-4 ring-white">
                                {profile.name.charAt(0)}
                            </div>
                            <button className="absolute bottom-0 right-0 p-2 bg-[#63C132] text-white rounded-full shadow-lg hover:bg-[#52A428] transition-all">
                                <Camera size={16} />
                            </button>
                        </div>
