...
import API from '../api';
import { Clock, Download, Filter, Search, Calendar, User, ChevronRight, BarChart3, TrendingUp, FolderOpen, Tag } from 'lucide-react';
        ...
        const fetchData = async () => {
            try {
                const [logsRes, statsRes, analysisRes] = await Promise.all([
                    API.get('/team-lead/worklogs'),
                    API.get('/team-lead/worklogs/stats'),
                    API.get('/team-lead/worklogs/analysis')
                ]);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white rounded-[32px] p-6 shadow-sm border border-slate-50 lg:col-span-3">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h3 className="text-sm font-black text-[#0B3C5D] uppercase tracking-widest">Project Distribution</h3>
                        </div>
                        <div className="h-[250px] w-full">
                            {analysis.topProjects?.length > 0 ? (
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={analysis.topProjects} margin={{ top: 10, right: 10, left: -20, bottom: 0 }} barSize={32}>
                                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                                        <XAxis
                                            dataKey="name"
                                            axisLine={false}
